...
public class ClientViewController implements Initializable {
    ...
    void setBtPlayMusic(ActionEvent e){
        Media sound = new Media(decodedPath+"jingle.mp3");
        if (mediaPlayer != null) {
            mediaPlayer.stop();
        }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        String path = ClientViewController.class.getProtectionDomain().getCodeSource().getLocation().getPath();
        String parentFolder = new File(path).getParent();
        try {
            decodedPath = URLDecoder.decode(parentFolder, "UTF-8");
        ...
        username = "Default";
        userImage= decodedPath + "images/logo.png";
        userStatus = "Online";
